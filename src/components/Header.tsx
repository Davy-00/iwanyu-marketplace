        ...
        <div className="container mx-auto px-4 py-3">
          <div className="flex items-center gap-4 lg:gap-8">
              ...
              <form onSubmit={handleSearchSubmit} className="w-full flex group">
                <div className="relative flex-1">
                  <Input
                    ...
                    className="h-11 w-full rounded-l-lg rounded-r-none border-0 bg-white pl-4 pr-4 text-sm text-iwanyu-foreground placeholder:text-gray-400 focus-visible:ring-2 focus-visible:ring-iwanyu-primary focus-visible:ring-offset-0"
                  />
                </div>
                <Button 
                  type="submit" 
                  className="h-11 px-5 rounded-l-none rounded-r-lg bg-gradient-to-r from-iwanyu-primary to-iwanyu-primary-glow text-iwanyu-dark font-semibold hover:brightness-110 transition-all shadow-[0_0_20px_rgba(255,153,0,0.3)] hover:shadow-[0_0_25px_rgba(255,153,0,0.5)]"
                >
            ...
            <div className="hidden md:flex items-center gap-1">
              <Link 
                to="/account" 
                className="flex items-center gap-2 px-3 py-2 rounded-lg text-white/80 hover:text-white hover:bg-white/5 transition-all group"
              >
              ...
              <Link 
                to="/wishlist" 
                className="relative p-2 rounded-lg text-white/80 hover:text-white hover:bg-white/5 transition-all group"
              >
              ...
              <Link 
                to="/cart" 
                className="relative flex items-center gap-1 px-3 py-2 rounded-lg text-white/80 hover:text-white hover:bg-white/5 transition-all group"
              >
                <div className="relative">
          ...
          <div className="mt-3 md:hidden">
            <form onSubmit={handleSearchSubmit} className="flex">
              <Input
                ...
                value={searchQuery}
                onChange={handleSearchChange}
                className="h-10 flex-1 rounded-l-lg rounded-r-none border-0 bg-white text-iwanyu-foreground placeholder:text-gray-400"
              />
              <Button 
                type="submit" 
                className="h-10 px-4 rounded-l-none rounded-r-lg bg-gradient-to-r from-iwanyu-primary to-iwanyu-primary-glow text-iwanyu-dark font-semibold"
              >
        ...
        <div className="container mx-auto hidden px-4 md:block">
          <ul className="flex items-center gap-1 text-sm py-1.5 overflow-x-auto">
            ...
            {categories.slice(0, 6).map((category) => (
              <li key={category.id}>
                <Link 
                  ...
                  className="block px-3 py-1.5 rounded-md text-white/70 hover:text-white hover:bg-white/10 transition-all text-sm"
                >
                  {category.name}
                </Link>
              </li>
            ...
            <li>
              <Link 
                to="/deals" 
                className="flex items-center gap-1 px-3 py-1.5 rounded-md bg-gradient-to-r from-iwanyu-primary/20 to-transparent text-iwanyu-primary hover:from-iwanyu-primary/30 transition-all font-semibold text-sm"
              >
                ...
                <ul className="space-y-1">
                  {categories.map((category) => (
                    <li key={category.id}>
                      <Link 
                        to={`/category/${category.id}`} 
                        className="flex items-center justify-between py-2.5 px-3 rounded-lg text-iwanyu-foreground hover:bg-iwanyu-muted transition-colors"
                        onClick={toggleMobileMenu}
                      >
              ...
              <div className="p-4">
                ...
                <ul className="space-y-1">
                  <li>
                    <Link 
                      to="/account" 
                      className="flex items-center gap-3 py-2.5 px-3 rounded-lg text-iwanyu-foreground hover:bg-iwanyu-muted transition-colors"
                      onClick={toggleMobileMenu}
                    >
